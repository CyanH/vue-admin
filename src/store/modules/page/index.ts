...
const usePageStore = defineStore('page', {
  ...
  getters: {
    pageSetting(state: pageState): pageState {
      return { ...state };
    },
  },
  ...
  actions: {
     ...
     */
    async openedLoad() {
      const value = await Cookies.get('opened')
      ...
      const valid: number[] = []
      // 处理数据
      this.opened = value.map((opened: { fullPath: string; name: any; }) => {
        // 忽略首页
      ...
      }).filter((opened: any, index: number) => valid[index] === 1)
     ...
     */
    async open({ name, params, query, fullPath, meta }: { name: any, params: any, query: any, fullPath: any, meta: any }) {
      // 已经打开的页面
      ...
      if (isKeepAlive({ meta })) this.keepAlivePush(name)
      // 设置当前的页面
     ...
     */
    async openedUpdate({ index, params, query, fullPath }: { index: number, params: any, query: any, fullPath: any }) {
      // 更新页面列表某一项
      ...
      // 持久化
      await Cookies.set('opened', this.opened)
    },
    /**
     * @class opened
     ...
     */
    async add({ tag, params, query, fullPath }: { tag: any, params: any, query: any, fullPath: any }) {
     ...
     */
    async openedSort(oldIndex: number, newIndex: number) {
      // 重排页面列表某一项
     ...
     */
    async close(tagName: string) {
      // 预定下个新页面
      let newPage: NewPage = { name: '', params: {}, query: {} };
      ...
      if (isCurrent) {
        ...
        const routerObj = <RouteLocationRaw>{ name, params, query }
     ...
     */
    async closeLeft(tagName: string) {
      const pageAim = tagName || this.current
      let currentIndex = 0
      ...
      if (currentIndex > 0) {
        ...
        for (let i = this.opened.length - 1; i >= 0; i--) {
          if (this.opened[i].name === 'index' || i >= currentIndex) continue
     ...
     * @param {Object} payload { pageSelect: 当前选中的tagName }
     */
    async closeRight(tagName: string) {
      const pageAim = tagName || this.current
      let currentIndex = 0
      this.opened.forEach((page, index) => {
        if (page.fullPath === pageAim) currentIndex = index
      ...
      // 删除打开的页面 并在缓存设置中删除
      for (let i = this.opened.length - 1; i >= 0; i--) {
        if (this.opened[i].name === 'index' || currentIndex >= i) continue
        this.keepAliveRemove(this.opened[i].name)
        this.opened.splice(i, 1)
      }
      // 持久化
     ...
     * @param {Object} payload { pageSelect: 当前选中的tagName }
     */
    async closeOther(tagName: string) {
      const pageAim = tagName || this.current
      let currentIndex = 0
      this.opened.forEach((page, index) => {
        if (page.fullPath === pageAim) currentIndex = index
    ...
    async closeAll() {
      // 删除打开的页面 并在缓存设置中删除
      for (let i = this.opened.length - 1; i >= 0; i--) {
        if (this.opened[i].name === 'index') continue
        this.keepAliveRemove(this.opened[i].name)
        this.opened.splice(i, 1)
      }
      // 持久化
